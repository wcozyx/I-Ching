...
#由筮卜的卦数转化为本卦（卦数七、九为阳，卦数六、八为阴）
def line(i, linenum):
    if linenum == 7 or linenum == 9:
        return 1
    else:
        return 0

#由筮卜的卦数转化为之卦（将卦数为九的变为阴，将卦数为六的变为阳）
def zline(i,linenum):
  if linenum == 8 or linenum == 9:
      return 0
  if linenum == 6 or linenum == 7:
      return 1

##开始占卜
#定义卦数列表、本卦六爻列表及之卦六爻列表
linenums = []
lines = []
zlines = []
...
trigramt = lines[3],lines[4],lines[5]
...
for k in range(6):
  if linenums[k] == 6 or linenums[k] == 9:
    bline_list.append(k)
...
hexagram = trigram[trigramt], trigram[trigramb]
zhexagram = trigram[ztrigramt], trigram[ztrigramb]

bengua = trigram[trigramt]+trigram[trigramb]+trigram_def[trigram[trigramt]]+trigram_def[trigram[trigramb]]+"："+hexagram_def[hexagram]
zhigua = trigram[ztrigramt]+trigram[ztrigramb]+trigram_def[trigram[ztrigramt]]+trigram_def[trigram[ztrigramb]]+"："+hexagram_def[zhexagram]
