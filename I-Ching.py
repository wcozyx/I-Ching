...
TOTAL_STICKS = 50
TAI_JI = 1
INITIAL_STICKS = TOTAL_STICKS - TAI_JI  # 49

# ------------------------ Helper Functions ------------------------

def split_sticks(total: int) -> Tuple[int, int]:
    """
    Split a pile of sticks into two non‑empty piles.
    """
...
def extra_sticks(remainder: int) -> int:
    """
    In the I Ching procedure, if a remainder is 0, we count it as 4.
    """
    return 4 if remainder == 0 else remainder


def perform_one_change(sticks: int) -> int:
    """
    Perform one 'change' (three changes make one line).
    Returns the number of sticks left after this change.
    """
    left, right = split_sticks(sticks)
    # Take one stick from the left pile
    left_remainder = (left - 1) % 4
    right_remainder = right % 4
    # Remove the taken stick and the extra remainders
    return sticks - 1 - extra_sticks(left_remainder) - extra_sticks(right_remainder)


def perform_three_changes() -> int:
    """
    Perform the full three‑change process for a single line.
    Returns the line number (6, 7, 8, or 9).
    """
    sticks = INITIAL_STICKS
    for _ in range(3):
        sticks = perform_one_change(sticks)
    return sticks // 4  # 36→9, 32→8, 28→7, 24→6


def get_line_number_to_binary(line_num: int) -> int:
    """Map line number to binary: 7 or 9 → 1 (yang), 6 or 8 → 0 (yin)."""
    return 1 if line_num in (7, 9) else 0


def get_line_number_to_changing(line_num: int) -> int:
    """Map line number to the 'changing' line value for the '之卦'."""
    # For 之卦: 9 becomes 0, 6 becomes 1, others unchanged (8→0, 7→1)
    if line_num in (8, 9):
        return 0
    else:  # 6 or 7
        return 1
...
HEXAGRAM_EX = {
    ...
    ("☱", "☶"): "咸，亨，利貞，取女吉。初六，咸其拇。六二，咸其腓，凶，居吉。九三，咸其股，執其隨，往吝。九四，貞吉，悔亡，憧憧往來，朋從爾思。九五，咸其脢，无悔。上六，咸其輔頰舌。",
    ...
    ("☰", "☴"): "姤，女壯，勿用取女。初六，繫于金柅，貞吉。有攸往，見凶。羸豕孚蹢躅。九二，包有魚，无咎，不利賓。九三，臀无膚，其行次且，厲，无大咎。九四，包無魚，起凶。九五，以杞包瓜，含章，有隕自天。上九，姤其角，吝，无咎。",
    ...
    ("☳", "☶"): "小過，亨，利貞。可小事，不可大事。飛鳥遺之音，不宜上，宜下。大吉。初六，飛鳥以凶。六二，過其祖，遇其妣，不及其君，遇其臣，无咎。九三，弗過防之，從或戕之，凶。九四，无咎，弗過遇之，往厲必戒，勿用永貞。六五，密雲不雨，自我西郊，公弋取彼在穴。上九，弗遇過之，飛鳥離之，凶，是謂災眚。",
...
def get_trigram_from_lines(lines: Tuple[int, int, int]) -> str:
    """Convert a tuple of three binary lines (0/1) into a trigram symbol."""
    return TRIGRAM_SYMBOLS[lines]


def get_hexagram_name_and_explanation(upper: str, lower: str) -> Tuple[str, str]:
    """Return (name, explanation) for a hexagram given its upper and lower trigram symbols."""
    key = (upper, lower)
    return HEXAGRAM_DEF.get(key, "未知卦"), HEXAGRAM_EX.get(key, "無解釋")


def get_changing_positions(line_numbers: List[int]) -> List[int]:
    """Return 1‑based positions of lines that are 6 or 9 (changing lines)."""
    return [i + 1 for i, num in enumerate(line_numbers) if num in (6, 9)]
...
def main() -> None:
    ...
    # Generate six line numbers
    line_numbers: List[int] = []
    for _ in range(6):
        line_numbers.append(perform_three_changes())

    # Map each line number to binary (0/1) for 本卦
    ben_lines = [get_line_number_to_binary(n) for n in line_numbers]
    # Map to 之卦 binary (changing lines flipped)
    zhi_lines = [get_line_number_to_changing(n) for n in line_numbers]

    # Split into lower and upper trigrams (lower = first three, upper = last three)
    ben_lower = tuple(ben_lines[0:3])
    ben_upper = tuple(ben_lines[3:6])
    zhi_lower = tuple(zhi_lines[0:3])
    zhi_upper = tuple(zhi_lines[3:6])

    # Get trigram symbols
    ben_upper_sym = get_trigram_from_lines(ben_upper)
    ben_lower_sym = get_trigram_from_lines(ben_lower)
    zhi_upper_sym = get_trigram_from_lines(zhi_upper)
    zhi_lower_sym = get_trigram_from_lines(zhi_lower)

    # Get hexagram info
    ben_name, ben_ex = get_hexagram_name_and_explanation(ben_upper_sym, ben_lower_sym)
    zhi_name, zhi_ex = get_hexagram_name_and_explanation(zhi_upper_sym, zhi_lower_sym)

    # Find changing lines
    changing = get_changing_positions(line_numbers)
    changing_str = "、".join(str(p) for p in changing) if changing else "無"

    # Determine interpretation rule based on number of changing lines
    num_changing = len(changing)
    rule_map = {
        0: "六爻皆不变者，只有本卦而无之卦，参考本卦卦辞。",
        1: "一爻变者，参考本卦变爻的爻辞。",
        2: "二爻变者，参考本卦二个变爻的爻辞，但以上爻为主。",
        3: "三爻变者，参考本卦及之卦的卦辞，但以本卦为主。",
        4: "四爻变者，参考之卦中不变之爻的爻辞，但以下爻为主。",
        5: "五爻变者，参考之卦中不变之爻的爻辞。",
        6: "六爻变者，参考乾坤而卦的用（九、六）辞，并参考之卦卦辞。"
    }
    rule = rule_map.get(num_changing, "")

    # Output results
    print("=" * 50)
    print("卦數：", line_numbers)
    print("本卦（二進位）：", ben_lines)
    print("之卦（二進位）：", zhi_lines)
    print("\n本卦上卦：", ben_upper_sym, "（", TRIGRAM_NAMES[ben_upper_sym], "）")
    print("本卦下卦：", ben_lower_sym, "（", TRIGRAM_NAMES[ben_lower_sym], "）")
    print("之卦上卦：", zhi_upper_sym, "（", TRIGRAM_NAMES[zhi_upper_sym], "）")
    print("之卦下卦：", zhi_lower_sym, "（", TRIGRAM_NAMES[zhi_lower_sym], "）")
    print("\n變爻位置：", changing_str)

    print("\n" + "=" * 50)
    print("本卦")
    print(f"{ben_upper_sym}{ben_lower_sym} {ben_name}")
    print(ben_ex)

    print("\n" + "=" * 50)
    print("之卦")
    print(f"{zhi_upper_sym}{zhi_lower_sym} {zhi_name}")
    print(zhi_ex)
